...
pub type XonThresholdW<'a, REG> = crate::FieldWriter<'a, REG, 8>;
...
pub type XoffThresholdW<'a, REG> = crate::FieldWriter<'a, REG, 8>;
impl R {
    ...
    pub fn xon_threshold(&self) -> XonThresholdR {
        XonThresholdR::new((self.bits & 0xff) as u8)
    }
    ...
    pub fn xoff_threshold(&self) -> XoffThresholdR {
        XoffThresholdR::new(((self.bits >> 8) & 0xff) as u8)
    }
}
impl W {
    #[doc = "Bits 0:7 - When the data amount in Rx-FIFO is less than this register value with uart_sw_flow_con_en set to 1 it will send a Xon char."]
