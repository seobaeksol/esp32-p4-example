...
impl R {
    ...
    #[inline(always)]
    pub fn wb_mode(&self) -> WbModeR {
        WbModeR::new(((self.bits >> 16) & 0xff) as u8)
    }
}
impl W {
    #[doc = "Bits 0:15 - The value is stored when set spi_mem_flash_rdsr bit and spi_mem_flash_res bit."]
    #[inline(always)]
    pub fn status(&mut self) -> StatusW<'_, RdStatusSpec> {
        StatusW::new(self, 0)
    }
    #[doc = "Bits 16:23 - Mode bits in the flash fast read mode it is combined with spi_mem_fastrd_mode bit."]
    #[inline(always)]
    pub fn wb_mode(&mut self) -> WbModeW<'_, RdStatusSpec> {
        WbModeW::new(self, 16)
    }
}
