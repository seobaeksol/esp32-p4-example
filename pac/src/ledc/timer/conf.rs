...
pub type DutyResR = crate::FieldReader;
#[doc = "Field `DUTY_RES` writer - Configures the range of the counter in timer %s."]
pub type DutyResW<'a, REG> = crate::FieldWriter<'a, REG, 5>;
...
pub type TickSelR = crate::BitReader;
#[doc = "Field `TICK_SEL` writer - Configures which clock is timer %s selected. Unused."]
pub type TickSelW<'a, REG> = crate::BitWriter<'a, REG>;
...
impl R {
    ...
    #[inline(always)]
    pub fn duty_res(&self) -> DutyResR {
        DutyResR::new((self.bits & 0x1f) as u8)
    }
    ...
    #[inline(always)]
    pub fn tick_sel(&self) -> TickSelR {
        TickSelR::new(((self.bits >> 25) & 1) != 0)
    }
}
impl W {
    #[doc = "Bits 0:4 - Configures the range of the counter in timer %s."]
    #[inline(always)]
    pub fn duty_res(&mut self) -> DutyResW<'_, ConfSpec> {
        DutyResW::new(self, 0)
    }
    #[doc = "Bits 5:22 - Configures the divisor for the divider in timer %s.The least significant eight bits represent the fractional part."]
    #[inline(always)]
    pub fn clk_div(&mut self) -> ClkDivW<'_, ConfSpec> {
        ClkDivW::new(self, 5)
    }
    #[doc = "Bit 23 - Configures whether or not to pause the counter in timer %s.\\\\0: Normal\\\\1: Pause"]
    #[inline(always)]
    pub fn pause(&mut self) -> PauseW<'_, ConfSpec> {
        PauseW::new(self, 23)
    }
    #[doc = "Bit 24 - Configures whether or not to reset timer %s. The counter will show 0 after reset.\\\\0: Not reset\\\\1: Reset"]
    #[inline(always)]
    pub fn rst(&mut self) -> RstW<'_, ConfSpec> {
        RstW::new(self, 24)
    }
    #[doc = "Bit 25 - Configures which clock is timer %s selected. Unused."]
    #[inline(always)]
    pub fn tick_sel(&mut self) -> TickSelW<'_, ConfSpec> {
        TickSelW::new(self, 25)
    }
    ...
    #[inline(always)]
    pub fn para_up(&mut self) -> ParaUpW<'_, ConfSpec> {
        ParaUpW::new(self, 26)
    }
}
