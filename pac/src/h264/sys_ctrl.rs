...
pub type DmaMoveStartW<'a, REG> = crate::BitWriter<'a, REG>;
...
pub type FrameModeR = crate::BitReader;
...
impl R {
    ...
    #[inline(always)]
    pub fn frame_mode(&self) -> FrameModeR {
        FrameModeR::new(((self.bits >> 2) & 1) != 0)
    }
}
impl W {
    #[doc = "Bit 0 - Configures whether or not to start encoding one frame.\\\\0: Invalid. No effect\\\\1: Start encoding one frame"]
    #[inline(always)]
    pub fn frame_start(&mut self) -> FrameStartW<'_, SysCtrlSpec> {
    ...
    #[inline(always)]
    pub fn dma_move_start(&mut self) -> DmaMoveStartW<'_, SysCtrlSpec> {
        DmaMoveStartW::new(self, 1)
    }
    #[doc = "Bit 2 - Configures H264 running mode. When field H264_DUAL_STREAM_MODE is set to 1, this field must be set to 1 too.\\\\0: GOP mode. Before every GOP first frame start, need reconfig reference frame DMA\\\\1: Frame mode. Before every frame start, need reconfig reference frame DMA"]
    #[inline(always)]
    pub fn frame_mode(&mut self) -> FrameModeW<'_, SysCtrlSpec> {
        FrameModeW::new(self, 2)
    }
    #[doc = "Bit 3 - Configures whether or not to reset H264 ip.\\\\0: Invalid. No effect\\\\1: Reset H264 ip"]
    #[inline(always)]
    pub fn sys_rst_pulse(&mut self) -> SysRstPulseW<'_, SysCtrlSpec> {
        SysRstPulseW::new(self, 3)
    }
}
