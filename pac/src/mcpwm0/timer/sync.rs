...
pub type PhaseDirectionR = crate::BitReader;
...
pub type PhaseDirectionW<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ...
    pub fn synco_sel(&self) -> SyncoSelR {
        SyncoSelR::new(((self.bits >> 2) & 3) as u8)
    }
    #[doc = "Bits 4:19 - Configures the phase for timer%s reload on sync event."]
    #[inline(always)]
    pub fn phase(&self) -> PhaseR {
        PhaseR::new(((self.bits >> 4) & 0xffff) as u16)
    }
    #[doc = "Bit 20 - Configures the PWM timer%s's direction when timer%s mode is up-down mode.\\\\0: Increase\\\\1: Decrease"]
    #[inline(always)]
    pub fn phase_direction(&self) -> PhaseDirectionR {
        PhaseDirectionR::new(((self.bits >> 20) & 1) != 0)
    }
}
impl W {
    #[doc = "Bit 0 - Configures whether or not to enable timer%s reloading with phase on sync input event is enabled.\\\\0: Disable\\\\1: Enable"]
    #[inline(always)]
    pub fn synci_en(&mut self) -> SynciEnW<'_, SyncSpec> {
        SynciEnW::new(self, 0)
    }
    #[doc = "Bit 1 - Configures the generation of software sync. Toggling this bit will trigger a software sync."]
    #[inline(always)]
    pub fn sw(&mut self) -> SwW<'_, SyncSpec> {
        SwW::new(self, 1)
    }
    #[doc = "Bits 2:3 - Configures the selection of PWM timer%s sync_out.\\\\0: Sync_in\\\\1: TEZ\\\\2: TEP\\\\3: Invalid, sync_out selects noting"]
    #[inline(always)]
    pub fn synco_sel(&mut self) -> SyncoSelW<'_, SyncSpec> {
        SyncoSelW::new(self, 2)
    }
    #[doc = "Bits 4:19 - Configures the phase for timer%s reload on sync event."]
    #[inline(always)]
    pub fn phase(&mut self) -> PhaseW<'_, SyncSpec> {
        PhaseW::new(self, 4)
    }
    #[doc = "Bit 20 - Configures the PWM timer%s's direction when timer%s mode is up-down mode.\\\\0: Increase\\\\1: Decrease"]
    #[inline(always)]
    pub fn phase_direction(&mut self) -> PhaseDirectionW<'_, SyncSpec> {
        PhaseDirectionW::new(self, 20)
    }
}
